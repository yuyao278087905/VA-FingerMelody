...
public class CircleAnnotHandler implements AnnotHandler {
    ...
    public void onColorValueChanged(int color) {
        ...
        try {
            if (annot != null && ToolUtil.getCurrentAnnotHandler(uiExtensionsManager) == this && color != annot.getBorderColor()) {
                modifyAnnot(annot.getPage().getIndex(), annot, annot.getRect(), color, (int)(((Circle)annot).getOpacity() * 255f), annot.getBorderInfo().getWidth(), annot.getContent(), false, null);
            }
    ...
    public void onLineWidthValueChanged(float lineWidth) {// to be continued
        ...
        try {
            if (annot != null && ToolUtil.getCurrentAnnotHandler(uiExtensionsManager) == this && lineWidth != annot.getBorderInfo().getWidth()) {
                ...
                modifyAnnot(annot.getPage().getIndex(), annot, bboxRect, (int)annot.getBorderColor(), (int)(((Circle)annot).getOpacity() * 255f) , lineWidth, annot.getContent(), false, null);
    ...
    public void onAnnotDeselected(Annot annot, boolean needInvalid) {
        ...
        try {
            ...
            if (needInvalid && mIsModify) {
                ...
                if (mTempLastColor == annot.getBorderColor() && mTempLastLineWidth == annot.getBorderInfo().getWidth() && mTempLastBBox.equals(annot.getRect()) && mTempLastOpacity == (int) (((Circle)annot).getOpacity() * 255f)) {
                    modifyAnnot(page.getIndex(), annot, annot.getRect(), (int) annot.getBorderColor(), (int) (((Circle)annot).getOpacity() * 255f), annot.getBorderInfo().getWidth(), annot.getContent(), false, null);
                } else {
                    modifyAnnot(page.getIndex(), annot, annot.getRect(), (int) annot.getBorderColor(), (int) (((Circle)annot).getOpacity() * 255f), annot.getBorderInfo().getWidth(), annot.getContent(), true, null);
                }
            } else if (mIsModify) {
    ...
    public void addAnnot(int pageIndex, AnnotContent contentSupplier, final Event.Callback result) {
        ...
        try {
            ...
            Annot annot = page.addAnnot(Annot.e_annotCircle, contentSupplier.getBBox());
            annot.setUniqueID(contentSupplier.getNM());
            annot.setBorderColor(contentSupplier.getColor());
            ((Circle) annot).setOpacity(contentSupplier.getOpacity() / 255f);
            if (contentSupplier.getLineWidth() != 0) {
                BorderInfo borderInfo = new BorderInfo();
                borderInfo.setWidth(contentSupplier.getLineWidth());
                annot.setBorderInfo(borderInfo);
            }
            ((Circle) annot).setCreationDateTime(AppDmUtil.currentDateToDocumentDate());
            annot.setModifiedDateTime(AppDmUtil.currentDateToDocumentDate());
                ...
                case MotionEvent.ACTION_CANCEL:
                    if (mTouchCaptured && annot == DocumentManager.getInstance(mPdfViewCtrl).getCurrentAnnot() && pageIndex == annot.getPage().getIndex()) {
                        ...
                        if (mLastOper != OPER_DEFAULT && !mDownPoint.equals(mLastPoint.x, mLastPoint.y)) {
                            ...
                            modifyAnnot(pageIndex, annot, bboxRect, (int)annot.getBorderColor(), (int)(((Circle)annot).getOpacity() * 255f), _lineWidth, annot.getContent(), false, null);

    ...
    public void loadText() {
        ...
        if (mTextContent != null && !mTextContent.equals("")) {
            if (mEditState) {
                ...
                int iStart = 0;
                int count = mTextContent.length();
                for (int i = 0; i < mTextContent.length(); i++) {
                    char ch = mTextContent.charAt(i);
                    if (ch == '\n' || ch == '\r') {
                        mStringList.add(mTextContent.substring(iStart, i + 1));
                        iStart = i + 1;
                        if (i == count - 1) {
                            mStringList.add("");
                        }
                ...
                mStringList.add(mTextContent.substring(iStart, count));
            }
